...
pAuraHandler AuraHandler[TOTAL_AURAS] =
{
    ...
    &Aura::HandleAuraModBlockPercent,                       // 51 SPELL_AURA_MOD_BLOCK_CHANCE_PERCENT
...
Aura::Aura(SpellEntry const* spellproto, SpellEffectIndex eff, int32* currentBasePoints, SpellAuraHolder* holder, Unit* target, Unit* caster, Item* castItem) :
    ...
    m_isPersistent(false), m_spellAuraHolder(holder), m_scriptRef(this, NoopAuraDeleter())
{
...
Aura* CreateAura(SpellEntry const* spellproto, SpellEffectIndex eff, int32* currentBasePoints, SpellAuraHolder* holder, Unit* target, Unit* caster, Item* castItem)
{
    ...
    if(SpellEntry const* triggeredSpellInfo = sSpellTemplate.LookupEntry<SpellEntry>(spellproto->EffectTriggerSpell[eff]))
    {
...
void Aura::HandleAddModifier(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (spellProto->SpellFamilyName == SPELLFAMILY_WARLOCK && spellProto->SpellIconID == 3169)
        {
            ...
            const_cast<SpellEntry*>(spellProto)->SpellFamilyFlags = ClassFamilyMask(uint64(0x0000010000000002));
        }
        // Improved Flametongue Weapon, overwrite wrong data, maybe time re-add table
        else if (spellProto->Id == 37212)
        {
            // Flametongue Weapon (Passive)
            const_cast<SpellEntry*>(spellProto)->SpellFamilyFlags = ClassFamilyMask(uint64(0x0000000000200000));
        }
    }
                ...
                switch (auraId)
                {
                    case 28820:                             // Lightning Shield (The Earthshatterer set trigger after cast Lighting Shield)
                    {
                        ...
                        for (Unit::SpellAuraHolderMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                        {
                            ...
                            if (spell->IsFitToFamily(SPELLFAMILY_SHAMAN, uint64(0x0000000000000400)))
                                return;
                        }
                        ...
                        if ((*itr)->GetSpellProto()->SpellFamilyName==SPELLFAMILY_WARRIOR && (*itr)->GetSpellProto()->SpellIconID == 2775)
                        {
                            switch ((*itr)->GetSpellProto()->Id)
                            {
                                case 46859:                 // Unrelenting Assault, rank 1
                                    target->CastSpell(target, 64849, TRIGGERED_OLD_TRIGGERED, nullptr, (*itr));
                                    break;
                                case 46860:                 // Unrelenting Assault, rank 2
                                    target->CastSpell(target, 64850, TRIGGERED_OLD_TRIGGERED, nullptr, (*itr));
                                    break;
                                default:
                                    break;
                            }
            ...
            if (GetSpellProto()->SpellFamilyFlags & uint64(0x1000000000))
            {
                ...
                else
                {
                    ...
                    if (target->IsInWorld() && GetStackAmount() > 0)
                    {
                        ...
                        target->CastCustomSpell(target, 33778, &amount, nullptr, nullptr, TRIGGERED_OLD_TRIGGERED, nullptr, this, GetCasterGuid());
...
void Aura::HandleAuraMounted(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        // cast speed aura
        if (MountCapabilityEntry const* mountCapability = target->GetMountCapability(uint32(m_spellAuraHolder->GetSpellProto()->EffectMiscValueB[GetEffIndex()])))
            target->CastSpell(target, mountCapability->SpeedModSpell, TRIGGERED_OLD_TRIGGERED);
    }
    else
    {
        target->Unmount(true);
...
void Aura::HandleAuraModSkill(bool apply, bool /*Real*/)
{
    ...
    uint32 prot = m_spellAuraHolder->GetSpellProto()->EffectMiscValue[GetEffIndex()];
        ...
        if (Real)
        {
            ...
            if (target->GetAurasByType(SPELL_AURA_MOD_STEALTH).size() <= 1)
            {
                ...
                for (Unit::AuraList::const_iterator i = mDummyAuras.begin(); i != mDummyAuras.end(); ++i)
                {
                    ...
                    // Overkill
                    else if ((*i)->GetId() == 58426 && GetSpellProto()->SpellFamilyFlags & uint64(0x0000000000400000))
                    {
    ...
    if (!apply)
    {
        switch (GetId())
        {
            ...
            case 37670:                                     // Nether Charge Timer
                target->CastSpell(nullptr, GetSpellProto()->EffectTriggerSpell[GetEffIndex()], TRIGGERED_OLD_TRIGGERED);
                break;
...
void Aura::HandleModTotalPercentStat(bool apply, bool /*Real*/)
{
    ...
    if ((m_modifier.m_miscvalue & (1 << STAT_STAMINA)) && maxHPValue > 0 && GetSpellProto()->HasAttribute(SPELL_ATTR_ABILITY))
    {
...
// Exist more after, need add later
            ...
            if (GetEffIndex() > EFFECT_INDEX_0 && spell->EffectApplyAuraName[SpellEffectIndex(GetEffIndex()-1)] == SPELL_AURA_MOD_POWER_REGEN)
            {
                if (target->GetTypeId() != TYPEID_PLAYER)
                    return;
            ...
            switch (spell->Id)
            {
                case 55342:                                 // Mirror Image
                {
                    if (GetAuraTicks() != 1)
                        return;
                    if (Unit* pCaster = GetCaster())
                        pCaster->CastSpell(pCaster, GetSpellProto()->CalculateSimpleValue(m_effIndex), TRIGGERED_OLD_TRIGGERED, nullptr, this);
                    return;
                }
                default:
                    break;
...
//                return;
            // Hysteria
            if (spell->IsFitToFamilyMask(uint64(0x0000000020000000)))
            {
...
void Aura::HandlePreventFleeing(bool apply, bool Real)
{
    ...
    if (!fearAuras.empty())
    {
        ...
        if (apply)
            GetTarget()->SetFleeing(false, first->GetCasterGuid());
        else
    ...
    if (apply && (GetTarget()->GetTypeId() != TYPEID_PLAYER || !((Player*)GetTarget())->GetSession()->PlayerLoading()))
    {
        if (Unit* caster = GetCaster())
        {
            ...
            switch(GetSpellProto()->SpellFamilyName)
            {
                case SPELLFAMILY_MAGE:
                    {
                        if (GetSpellProto()->IsFitToFamilyMask(uint64(0x0000000000008000)))
                        {
                            // Mana Shield
                            // +50% from +spd bonus
...
bool Aura::HasMechanic(uint32 mechanic) const
{
    ...
    return GetSpellProto()->EffectMechanic[m_effIndex] == mechanic;
}
...
void SpellAuraHolder::HandleSpellSpecificBoosts(bool apply)
{
    ...
    switch(m_spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_HUNTER:
        {
            switch (GetId())
            {
                ...
                default:
                    ...
                    if (m_spellProto->IsFitToFamilyMask(uint64(0x0000000000000008)))
                    {
                        if (!apply)
                        {
                            Unit* caster = GetCaster();
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (m_spellProto->SpellFamilyFlags & uint64(0x0000000000000008)==0)
