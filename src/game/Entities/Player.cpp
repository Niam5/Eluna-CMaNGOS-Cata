...
Player::Player(WorldSession* session): Unit(), m_mover(this), m_camera(this), m_achievementMgr(this), m_reputationMgr(this)
{
    ...
    m_lastHonorKillsUpdateTime = time(nullptr);
...
void Player::GiveLevel(uint32 level)
{
    ...
    sObjectMgr.GetPlayerClassLevelInfo(plClass, level, basehp, basemana);
...
uint16 Player::GetSkillStep(uint16 skill) const
{
    ...
    return GetUInt16Value(PLAYER_SKILL_STEP_0 + skillStatus.pos / 2, skillStatus.pos & 1);
}
...
void Player::_ApplyItemBonuses(ItemPrototype const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    ScalingStatValuesEntry const* ssv = proto->StatScalingFactor ? sScalingStatValuesStore.LookupEntry(ssd_level) : nullptr;
...
void Player::_HandleDeadlyPoison(Unit* Target, WeaponAttackType attType, SpellEntry const* spellInfo)
{
    ...
    if (dPoison && dPoison->GetStackAmount() == spellInfo->StackAmount)
    {
        ...
        for (int s = 0; s < 3; ++s)
        {
            ...
            if (SpellEntry const* combatEntry = sSpellTemplate.LookupEntry<SpellEntry>(pSecondEnchant->spellid[s]))
                if (combatEntry->Dispel == DISPEL_POISON)
...
void Player::UpdateArmorSpecializations()
{
    ...
    for (int i = 0; i < MAX_ARMOR_SPECIALIZATION_SPELLS; ++i)
    {
        ...
        SpellEntry const * spellProto = sSpellTemplate.LookupEntry<SpellEntry>(armorSpecToTab[i].spellId);
