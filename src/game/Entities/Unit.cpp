...
bool Unit::haveOffhandWeapon() const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        return ((Player*)this)->GetWeaponForAttack(OFF_ATTACK, true, true) != nullptr;
    else
    {
...
void Unit::CalculateDamageAbsorbAndResist(Unit* pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32* absorb, uint32* resist, bool canReflect, bool ignoreResists, bool binary)
{
    ...
    if (pCaster != this)
    {
        AuraList const& vSplitDamageFlat = GetAurasByType(SPELL_AURA_SPLIT_DAMAGE_PCT);
...
void Unit::CalculateAbsorbResistBlock(Unit* pCaster, SpellNonMeleeDamage* damageInfo, SpellEntry const* spellProto, WeaponAttackType attType)
{
    ...
    {
        damageInfo->blocked = std::min(GetShieldBlockDamageValue(), damageInfo->damage);
    ...
    if (!heal)
    {
        ...
        switch (uint32(dmgClass))
        {
            ...
            case SPELL_DAMAGE_CLASS_RANGED:
                ...
                chance -= GetCombatRatingReduction(CR_RESILIENCE_DAMAGE_TAKEN);
                break;
            case SPELL_DAMAGE_CLASS_MAGIC:
    ...
    if (!heal)
    {
        ...
        switch (uint32(dmgClass))
        {
            ...
            case SPELL_DAMAGE_CLASS_RANGED:
                ...
                multiplier -= std::min(cap, (GetCombatRatingReduction(CR_RESILIENCE_DAMAGE_TAKEN) * factor)) * ratio;
                break;
            case SPELL_DAMAGE_CLASS_MAGIC:
...
float Unit::GetMissChance(const SpellEntry *entry, SpellSchoolMask schoolMask) const
{
    ...
    if (IsAreaOfEffectSpell(entry))
        chance += GetTotalAuraModifier(SPELL_AURA_160);
    return chance;
}
...
void Unit::_UpdateAutoRepeatSpell()
{
    ...
    // check movement
    if (IsMoving() && !HasAffectedAura(SPELL_AURA_ALLOW_CAST_WHILE_MOVING, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo))
    {
        // cancel wand shoot
        if (!isAutoShot)
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask)
{
    ...
    if (!mOverrideSpellPowerAuras.empty())
    {
        for (auto itr : mOverrideSpellPowerAuras)
            if (schoolMask & itr->GetModifier()->m_miscvalue)
    ...
    // pct spell power modifier
    Unit::AuraList const& mSpellPowerPctAuras = GetAurasByType(SPELL_AURA_MOD_INCREASE_SPELL_POWER_PCT);
    for (auto itr : mSpellPowerPctAuras)
    {
        if (!itr->GetModifier()->m_miscvalue || itr->GetModifier()->m_miscvalue & schoolMask)
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (getRace() == RACE_WORGEN && !IsInWorgenForm(true) && HasWorgenForm())
        CastSpell(this, 97709, TRIGGERED_OLD_TRIGGERED);   // cast Altered Form
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellEntry const* spellProto, SpellEffectIndex effect_index, int32 const* effBasePoints)
{
    switch (spellProto->EffectApplyAuraName[effect_index])
    {
        case SPELL_AURA_MOUNTED:
            if (MountCapabilityEntry const* mountCapability = GetMountCapability(uint32(spellProto->EffectMiscValueB[effect_index])))
    ...
    comboDamage = spellProto->EffectPointsPerComboPoint[effect_index];
    ...
    else
    {
        ...
        basePoints = effBasePoints ? *effBasePoints - 1 : spellProto->EffectBasePoints[effect_index];
...
int32 Unit::CalculateAuraDuration(SpellEntry const* spellProto, uint32 effectMask, int32 duration, Unit const* caster, Spell const* spell /*=NULL*/)
{
    ...
    if (caster == this)
    {
        switch(spellProto->SpellFamilyName)
        {
            case SPELLFAMILY_DRUID:
                // Thorns
                if (spellProto->SpellIconID == 53 && spellProto->IsFitToFamilyMask(uint64(0x0000000000000100)))
                {
                    // Glyph of Thorns
            ...
            case SPELLFAMILY_PALADIN:
                // Blessing of Might
                if (spellProto->SpellIconID == 298 && spellProto->IsFitToFamilyMask(uint64(0x0000000000000002)))
                {
                ...
                // Blessing of Wisdom
                else if (spellProto->SpellIconID == 306 && spellProto->IsFitToFamilyMask(uint64(0x0000000000010000)))
                {
